Satellite Monitoring for Forest Management Biomass Tool for ALOS
================================================================

The Biomass Tool for ALOS (BIOTA) tool is part of the `Satellite Monitoring for Forest Management (SMFM) project <https://www.smfm-project.com/`_ by the World Bank. It was developed by `LTS International <https://ltsi.co.uk/>`_ and the `University of Edinburgh <https://www.ed.ac.uk/geosciences>`_ with an integration in the SEPAL platform developed by the SEPAL developer team. The tool relies on the use of JAXA's `ALOS PALSAR <https://www.eorc.jaxa.jp/ALOS/en/about/palsar.htm>`_ L-band mosaics, allowing users to produce outputs of:
...
In this exercise, you will learn how to use the BIOTA tool to calculate above-ground biomass in dry forests and savannahs, as well as change maps.
...
.. tip::
    ...
    Depending on your computer screen size, sometimes the left column will appear on top of the content, as seen in the figure below.
    ...
    -   Keep the zoom level, but click outside of the column to hide it. Then, to open it again, you will need to select the three dots located on the upper-right side.
...
.. note::
    ...
    The BIOTA tool was designed for woodlands and dry forests, as it uses a generic equation to calibrate Gamma0 backscatter to forest AGB developed using forest plot data from Mozambique, Tanzania, and Malawi, in southern Africa. For global applicability, the tool supports the calibration of country-specific backscatter-AGB relationships through determined parameters, which will be explained later.
...
The tool gives you the option to choose a size of either a 1 x 1 degree tile or 5 x 5 degree tile. We will select the 1 x 1 tile size for time purposes.
